...
<div class="mb-4">
    ...
    <textarea 
        ...
        rows="{{ $rows }}"
        {!! $attributes->merge(['class' => 'block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed resize-vertical' . ($error ? ' border-red-500 focus:ring-red-500 focus:border-red-500' : '')]) !!}
    >{{ $slot }}</textarea>
